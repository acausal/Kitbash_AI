...
class SleepOrchestrator:
    ...
    def run_full_sleep_cycle(self) -> Dict[str, Any]:
        ...
        # Stage 1.5: Procedural Edge Extraction (Intra-Cartridge)
        print("\n[STAGE 1.5/6] Procedural Edge Extraction (Intra-Cartridge)")
        stage1_5_start = time.perf_counter()
        try:
            report['stages']['stage_1_5'] = self.run_stage_1_5()
            report['stages']['stage_1_5']['time_seconds'] = time.perf_counter() - stage1_5_start
            print(f"  ✓ Complete ({report['stages']['stage_1_5']['time_seconds']:.1f}s)")
        except Exception as e:
            print(f"  ✗ Failed: {e}")
            report['stages']['stage_1_5'] = {'error': str(e)}
        ...
        # Stage 2.5: Procedural Edge Extraction (Cross-Cartridge)
        print("\n[STAGE 2.5/6] Procedural Edge Extraction (Cross-Cartridge)")
        stage2_5_start = time.perf_counter()
        try:
            report['stages']['stage_2_5'] = self.run_stage_2_5()
            report['stages']['stage_2_5']['time_seconds'] = time.perf_counter() - stage2_5_start
            print(f"  ✓ Complete ({report['stages']['stage_2_5']['time_seconds']:.1f}s)")
        except Exception as e:
            print(f"  ✗ Failed: {e}")
            report['stages']['stage_2_5'] = {'error': str(e)}
        ...
        # ===== MUTATION 5: RECALIBRATION (Unified grain + edge feedback) =====
        print("\n[STAGE 5.5/6] Recalibration (Grain + Edge Feedback)")
        stage5_5_start = time.perf_counter()
        try:
            report['stages']['stage_5_5'] = self.run_stage_5_5()
            report['stages']['stage_5_5']['time_seconds'] = time.perf_counter() - stage5_5_start
            print(f"  ✓ Complete ({report['stages']['stage_5_5']['time_seconds']:.1f}s)")
        except Exception as e:
            print(f"  ✗ Failed: {e}")
            report['stages']['stage_5_5'] = {'error': str(e)}
    ...
    def run_stage_1_5(self) -> Dict[str, Any]:
        """
        Stage 1.5: Procedural Edge Extraction (Intra-Cartridge)
        
        Reads traces from dream bucket, extracts fact→fact edges within cartridges:
        - Parse query chains from traces.jsonl
        - Build edges from consecutive fact pairs in same cartridge
        - Calculate confidence-like weights from traversal frequency
        - Write procedural_edge_graph index
        
        Returns:
            Report dict with edge extraction statistics
        """
        from sleep_procedural_edge_extractor import ProceduralEdgeExtractor
        extractor = ProceduralEdgeExtractor(str(self.dream_bucket_dir))
        return extractor.extract_intra_cartridge_edges()
    ...
    def run_stage_2_5(self) -> Dict[str, Any]:
        """
        Stage 2.5: Procedural Edge Extraction (Cross-Cartridge)
        
        Reads traces again, extracts cartridge→cartridge navigation edges:
        - Identify fact pairs that cross cartridge boundaries
        - Build edges linking related domains
        - Update procedural_edge_graph with cross-cartridge linkages
        - Update metadata to reflect both intra and cross edges
        
        Returns:
            Report dict with cross-cartridge edge statistics
        """
        from sleep_procedural_edge_extractor import ProceduralEdgeExtractor
        extractor = ProceduralEdgeExtractor(str(self.dream_bucket_dir))
        return extractor.extract_cross_cartridge_edges()
    ...
    def run_stage_5_5(self) -> Dict[str, Any]:
        """
        Stage 5.5: Recalibration (Mutation 5 - Unified Feedback Loop)
        
        Apply Dream Bucket feedback uniformly to grain confidences and edge weights:
        - Read consistency violations from dream bucket
        - Identify affected grains and edges
        - Apply confidence penalties uniformly (same signal, same adjustment)
        - Save updated indices
        
        Treats procedural (edges) and declarative (grains) knowledge symmetrically.
        
        Returns:
            Report dict with recalibration statistics
        """
        from sleep_recalibration_service import RecalibrationService
        recalibrator = RecalibrationService(str(self.dream_bucket_dir))
        return recalibrator.run_recalibration_cycle()
