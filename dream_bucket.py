...
class DreamBucketWriter:
    ...
    def write_index(self, index_name: str, data: Dict[str, Any]) -> None:
        ...
        valid_indices = {
            ...
            "learning_summary", "system_health", "procedural_edge_graph"
        }
...
def log_trace(
    ...
    chain_type: str = "intra_query",
    session_id: Optional[str] = None,
    context: Optional[Dict[str, Any]] = None
) -> bool:
    """
    Convenience function to log a query chain (procedural trace).
    
    Args:
        writer: DreamBucketWriter instance
        query_id: Unique identifier for the query
        chain: List of dicts, each representing a step in the chain
               Each dict should have: 
               - 'grain_id' or 'fact_id': source element
               - 'traversal_type': how it was accessed (lookup, activation, etc.)
               - 'weight': optional salience weight
               - 'timestamp': optional step timestamp
        chain_type: "intra_query" (fact→fact) or "inter_query" (session-level, future)
        session_id: Optional session identifier
        context: Optional additional context (cartridge, hat, etc.)
