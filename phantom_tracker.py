...
class PhantomTracker:
    ...
    def _calculate_consistency(self, phantom: PhantomCandidate) -> None:
        ...
        # ===== RESONANCE WEB POLARITY (NEW - Mutation B) =====
        # Polarity reflects strength/weakness based on confidence and consistency
        # Warmth (+1): High avg confidence + high consistency = strong signal
        # Shadow (-1): Low confidence or high variance = weak/ambiguous signal
        # Neutral (0): Mixed signal
        
        avg_conf = phantom.avg_confidence()
        
        # Polarity combines confidence level and confidence consistency
        # High confidence + high consistency → warmth (+1)
        # Low confidence + high variance → shadow (-1)
        # Mixed → neutral (0)
        confidence_polarity = (avg_conf - 0.5) * 2.0  # Range: -1.0 to +1.0
        consistency_boost = (confidence_consistency - 0.5) * 0.6  # Boost/penalty based on consistency
        
        phantom.polarity = confidence_polarity + consistency_boost
        phantom.polarity = max(-1.0, min(1.0, phantom.polarity))  # Clamp to [-1, 1]
