...
- CartridgeLoader (fact lookup with Phase 1-4 learning: co-occurrence, anchors, CTR, seasonality)
- MTR 5.5 Engine (neural temporal reasoning with error signals)
- GrainRouter (crystallized grain cache with context-aware activation)
- ResonanceWeightService (Tier 5 temporal pattern tracking, Ebbinghaus decay)
- GrainSystem (phantom tracking → grain crystallization → persistence)
- Dream Bucket (persistent research signal logging for sleep pipeline)

Architecture:
1. Query arrives with context (text, user_id, session_id, hat/epistemic mode)
2. CartridgeLoader searches facts (learns co-occurrence, anchors, CTR, seasonality)
3. GrainRouter searches crystallized grains (L3 cache, sub-0.5ms if hit)
4. ResonanceWeightService records query pattern (Tier 5, Ebbinghaus decay)
5. MTR processes facts + grains with learned state
6. MTR error signal → phantom tracker records pattern (A)
7. Grain hit? → ResonanceWeightService reinforces pattern (B)
8. Every 51 queries → crystallization cycle (phantom→grain→L3 cache)
9. State saved: MTR weights + cartridge learning + grain registry + resonance patterns

Performance (Phase 3E):
- Cartridge lookup: 15-50ms
- Grain lookup: <1ms (L3 cache hit)
- MTR inference: 5-20ms
- Phantom tracking: <1ms
- Pattern reinforcement: <1ms
- Crystallization (every 51 queries): 200-500ms

Total end-to-end: 6-10ms (with learning)

Phase 5 Integration (Planned):
- BitNet specialist layer (fallback after MTR, before full LLM)
- BitNet triage uses ResonanceWeightService.get_active_patterns() for routing
- Learned triage agent replaces rule-based decisions

Author: Kitbash Team
Date: February 2026
Phase: 3E+ (QueryOrchestrator refactor)
"""
...
class QueryOrchestrator:
    """
    Main orchestration loop for query processing with full grain + resonance integration.
    
    Steps:
    1. User sends query with context (text, project, hat)
    2. CartridgeLoader searches facts (learns co-occurrence, anchors, CTR, seasonality)
    3. GrainRouter searches crystallized grains (L3 cache lookup)
    4. ResonanceWeightService records pattern (Tier 5, Ebbinghaus decay)
    5. MTR processes facts + grains with learned state
    6. MTR error signal → phantom tracker (via MTRGrainBridge)
    7. Grain hit? → ResonanceWeightService reinforces pattern
    8. Every 51 queries: crystallization cycle (phantom→grain)
    9. New grains loaded into GrainRouter cache
    10. State saved: MTR weights + cartridge learning + grain registry + resonance
    
    Architecture:
    - Stateful: MTR maintains weight matrices, phantom tracker maintains phantom history
    - Temporal: ResonanceWeightService tracks Tier 5 patterns with Ebbinghaus decay
    ...
    def __init__(self, 
        ...
        # ====================================================================
        # STEP 3.5: RESONANCE WEIGHT SERVICE (Tier 5, Temporal Pattern Tracking)
        # ====================================================================
        print("\n3.5. Initializing Resonance Weight Service...")
        self.resonance_service = ResonanceWeightService(
            initial_stability=3.0,
            stability_growth=2.0,
            cleanup_threshold=0.001,
            spacing_sensitive=False
        )
        print("   ✓ Resonance service initialized")
        print("   ✓ Ebbinghaus decay formula: weight = e^(-age / S)")
        print("   ✓ Patterns recorded on MTR hit, reinforced on grain hit")
        print("   ✓ Ready for BitNet triage integration (Phase 5)")
        
        # ====================================================================
        # STEP 5: GRAIN SYSTEM (Phantom tracking + crystallization)
        # ====================================================================
        ...
        if self.enable_grain_system:
            ...
            # ===== L2 WORKING THEORY SERVICE (Mutation 2) =====
            from l2_working_theory_service import L2WorkingTheoryService
            self.l2_service = L2WorkingTheoryService(
                dream_bucket_dir=dream_bucket_dir,
                grain_orchestrator=self.grain_orchestrator
            )
            print(f"   ✓ L2 Working Theory Service initialized (read-only audit)")
        else:
            print("\n5. Grain System: DISABLED (running MTR-only mode)")
            self.l2_service = None
        
        # ====================================================================
        # STEP 6: QUERY TRACKING
        # ====================================================================
        self.query_count = 0
    ...
    def process_query(self, context: QueryContext) -> QueryResult:
        ...
        # ====================================================================
        # PHASE 4.5: RESONANCE PATTERN TRACKING (Tier 5, Temporal)
        # ====================================================================
        # (A) Record query pattern on MTR inference (Tier 5 creation)
        query_pattern_hash = hashlib.sha256(context.query_text.encode()).hexdigest()
        mtr_confidence = 1.0 - float(error_signal.mean().clamp(0, 1))
        
        self.resonance_service.record_pattern(
            pattern_hash=query_pattern_hash,
            metadata={
                "query_text": context.query_text,
                "mtr_confidence": mtr_confidence,
                "concepts": context.query_text.lower().split()[:5],
                "context": context.project_context or "general",
            },
            initial_stability=3.0  # Start with default stability
        )
        
        # (B) Reinforce pattern if grain hit (Tier 5 reinforcement)
        # Spacing-sensitive: reinforcement of nearly-faded patterns gets bonus
        if grain_facts:
            self.resonance_service.reinforce_pattern(query_pattern_hash)
        ...
        # Advance resonance turn (called after each completed query)
        ...
        # ====================================================================
        # LOG PROCEDURAL TRACE (for sleep pipeline edge extraction)
        # ====================================================================
        if self.dream_bucket_writer is not None and (self.recent_facts or self.recent_grains):
            # Build chain from facts and grains accessed in this query
            chain = []
            
            # Add facts to chain
            for fact_id in self.recent_facts:
                chain.append({
                    'fact_id': fact_id,
            ...
            # Add grains to chain
            for grain_id in self.recent_grains:
                chain.append({
                    'grain_id': grain_id,
            ...
            # Log the trace
            if chain:
                from dream_bucket import log_trace
                log_trace(
                    self.dream_bucket_writer,
                    query_id=f"q_{self.query_count}_{int(start_time * 1000)}",
                    chain=chain,
    ...
    def print_summary(self) -> None:
        ...
        print("\n" + "="*70)
        print("QUERY ORCHESTRATOR SUMMARY")
        print("="*70)
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("QUERY ORCHESTRATOR INTEGRATION TEST - Full MTR + Cartridges + Grains + Resonance")
    print("="*70 + "\n")
