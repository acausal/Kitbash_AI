"""cli for tools.json_extractor. Usage:
    python -m tools.json_extractor input.json [-o out.md]
Exit 0 on success, 1 on failure (stderr message)."""
from __future__ import annotations
import argparse, os, sys
from . import convert_json_to_markdown

def main(argv=None) -> int:
    p = argparse.ArgumentParser(prog="json_extractor",
        description="JSON -> text from known fields (stdlib).")
    ...
    try:
        convert_json_to_markdown(a.input, out)
