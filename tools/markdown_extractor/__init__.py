"""markdown_extractor: markdown -> normalized pass-through text (stdlib only).
...
try:
    ...
    _logger = get_event_logger("markdown_extractor")
except Exception:
    _logger = None


def convert_markdown_to_markdown(input_path: str, output_path: str) -> None:
    if not os.path.isfile(input_path):
        raise FileNotFoundError(f"input file not found: {input_path}")
    low = input_path.lower()
    if not (low.endswith(".md") or low.endswith(".markdown")):
        raise ValueError(f"input is not a .md/.markdown file: {input_path}")
    ...
    # Preserve markdown syntax; only normalize line endings + blank lines.
    text = raw.replace("\r\n", "\n").replace("\r", "\n")
