"""odt_extractor: ODT -> clean text (odfpy).

Isolation-first tool (see tools/README.md). Allowed: odfpy + stdlib
...
try:
    ...
    _logger = get_event_logger("odt_extractor")
except Exception:
    _logger = None
...
def convert_odt_to_markdown(input_path: str, output_path: str) -> None:
    if not os.path.isfile(input_path):
        raise FileNotFoundError(f"input file not found: {input_path}")
    if not input_path.lower().endswith(_EXTS):
        raise ValueError(f"input is not a .odt file: {input_path}")
    ...
    try:
        ...
        parts = [teletype.extractText(p) for p in paras]
    except Exception as e:
        raise RuntimeError(f"ODT parse failed: {e}") from e

    text = _normalize("\n".join(parts))  # empty allowed
