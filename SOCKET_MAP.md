...
| Socket | Status | Notes |
|---|---|---|
| RedisBlackboard core | YELLOW | Clean API; **envelope + TTLs wired 2026-07-10** (SPEC §1/§4: `_wrap`/`_unwrap`, loud-reject on bad version, query 24h / health 5m / metrics 7d) — verified ad-hoc against fakeredis 14/14. Still YELLOW: **not wired into canonical path** (factory passes `redis_client=None`) and no §6 contract suite yet. |
...
| **GAP — Violation records carry no edge/fact-chain reference** | `data/subconscious/dream_bucket/live/violations.jsonl` | **BLOCKER (RED)** | Verified 2026-07-10: violation keys are `[dissonance_type, mtr_error_signal, returned_confidence, returned_fact_id, session_id, timestamp]` — **no `context`, no `recent_facts`, no edge ref**, despite `DREAM_BUCKET_DESIGN.md` Category C showing `recent_facts` in context. Blocks real F2 edge-targeting in Mutation 5 recalibration (the current stopgap no-ops because it can't resolve to a specific edge key). `recent_facts`/`context` must be populated by the writer before per-edge penalties are safe to apply. |
