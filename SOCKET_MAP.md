...
| Mutation 5 — Axiom-Aware Recalibration (`sleep_recalibration_service.py`) | Stage 5 `run_recalibration_cycle` | **YELLOW (was wrongly GREEN)** | **CORRECTION 2026-07-10 (SPEC_AXIOM_RECALIBRATION.md):** the prior GREEN was based on the file's docstring/uniform-treatment framing, NOT verified behavior. Verified reality: (F1) grain write-back is still a **no-op** — the registry save is commented out (lines 155–159) and grain penalties key on a synthetic `grain_from_fact_<id>` string (F3 blocked, see Grain schema gap). (F2 edge side) the "average penalty applied to every edge" logic has been **neutralized to a guarded no-op-and-report** (2026-07-10) — it no longer blanket-penalizes, but it is also NOT a real targeting fix. **F2 was believed to be causing live damage; it verifiably was not** — `procedural_edge_graph.json` does not exist on disk, so `_update_edge_weights` returned early and never touched edges. Real edge targeting (F2) remains BLOCKED on the violation-schema gap below. Spec closed partial: grain side + reconciliation (Steps 2–3) not done, blocked. |
