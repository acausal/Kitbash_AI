...
| InferenceEngine: BITNET | same (HTTP to local server) | **YELLOW→ (2026-07-11)** | Server DEPLOYED + reachable: `llama-server.exe` (llama.cpp build) runs `Little-Bitch-3B.i1-Q6_K.gguf` on 127.0.0.1:8080, answers raw `/completion` (verified independently, pre-orchestrator). `enable_bitnet=True` wires `BitNetEngine` into the registry (confirmed `['GRAIN','CARTRIDGE','BITNET']`). **BUT not yet GREEN:** `rule_based_triage.py` never includes BITNET in any `layer_sequence` ("BITNET deferred to Phase 4"), so the cascade stops at CARTRIDGE and BITNET is never attempted for a query. Engine + server verified; routing not yet. Adding BITNET to triage = cascade-ordering change, OUT OF SCOPE for Chat Milestone (flagged as follow-up). |
