...
| Intentional data stream format | **YELLOW (specified, wiring in progress)** | SPEC `docs/SPEC_STREAM_FORMAT.md` (envelope §1, registry §2, schemas §3, TTL §4). Part 1 landed 2026-07-10: envelope+TTLs in `redis_blackboard.py` (14/14 ad-hoc on fakeredis). Remaining for GREEN (§6 DONE-WHEN): GAP-1 coupling namespace fix, epistemic_snapshot writer, `TEST-bus_stream_format.py`, factory wiring. GAP-2 (versioning) resolved in blackboard; GAP-3 (TTLs) resolved. |
