...
| **GAP — Grain schema lacks `fact_id` AND `grain_type`** | on-disk grain files (`grains/*.json`) | **BLOCKER (RED) → RESOLVING via Mutation 1 (2026-07-10)** | Verified 2026-07-10: grains carry keys `[axiom_ids, cartridge_id, epistemic_level, grain_id, num_bits, quality_metrics, size_bytes, source_phantom_id, state, weight_distribution]` — **no `fact_id`, no `grain_type`**. Only `epistemic_level` (e.g. `L2_AXIOMATIC`) is a weak proxy for axiom/observation. Blocks: (F3/F1) fact→grain write-back in Mutation 5 recalibration, and (§2.2) axiom-vs-observation recalibration asymmetry. **Resolution in progress:** Mutation 1 adds `fact_ids` field + `from_dict` to `GrainMetadata` and flips `GrainRouter.grain_by_fact` to `Dict[int, List[str]]` (1:N accumulate; was 1:1 last-wins). Contract test `TEST-grain_format_contract.py` committed (`9b50ec8`), target 7/7. Grain type split (`grain_type`) already in `GrainMetadata` dataclass (Phase 5A) + persisted by `to_dict()`; F3 `fact_id` lookup is the missing piece. **Do NOT work around by content-matching.** Mutation 1 is the natural owner — see STATUS §C. |
