...
| **GAP — Grain schema lacks `fact_id` AND `grain_type`** | on-disk grain files (`grains/*.json`) | **BLOCKER (RED)** | Verified 2026-07-10: grains carry keys `[axiom_ids, cartridge_id, epistemic_level, grain_id, num_bits, quality_metrics, size_bytes, source_phantom_id, state, weight_distribution]` — **no `fact_id`, no `grain_type`**. Only `epistemic_level` (e.g. `L2_AXIOMATIC`) is a weak proxy for axiom/observation. Blocks: (F3/F1) fact→grain write-back in Mutation 5 recalibration, and (§2.2) axiom-vs-observation recalibration asymmetry. Resolution = a grain schema change (add `fact_id` + `grain_type`), tracked separately — NOT to be worked around by content-matching. Mutation 1 (L1/L2 axiom-vs-observation split) is the natural owner of the `grain_type` addition. |
