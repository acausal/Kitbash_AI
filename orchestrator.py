    ...
    def crystallize_grains(self, validated_phantoms: List[Dict], 
        ...
        for phantom_info in validated_phantoms:
            ...
            try:
                ...
                # ===== GRAIN TYPE DECISION WITH POLARITY (Mutation B - Resonance Web) =====
                # Confidence determines baseline; polarity (strength/weakness signal) adjusts
                # Warmth (+1): Strong signal → favor axiom even at lower confidence
                # Shadow (-1): Weak signal → favor observation despite high confidence
                # Neutral (0): Use confidence threshold
                
                confidence = phantom_info.get('confidence', phantom.avg_confidence())
                polarity = phantom.polarity if hasattr(phantom, 'polarity') else 0.0
                
                # Polarity-adjusted thresholds
                # High polarity (warmth): axiom threshold lowers to 0.90
                # Low polarity (shadow): axiom threshold raises to 1.0 (impossible, forces observation)
                axiom_threshold = 0.95 - (polarity * 0.05)  # Range: [0.90, 1.0]
                
                if confidence >= axiom_threshold:
                    grain_type = "axiom"
