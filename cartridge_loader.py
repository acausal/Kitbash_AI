...
class CartridgeLoader:
    ...
    def get_phase_stats(self) -> Dict[str, Any]:
        """
        Extract Phase 1–4 learning statistics from fact layer.
        
        Phase 1: Co-occurrence (fact relationships)
        Phase 2: Query Anchors (which queries match which facts)
        Phase 3: CTR (fact usefulness via confidence)
        Phase 4: Temporal Seasonality (temporal access patterns)
        
        Returns:
            Dict with phase-wise aggregated statistics for grain crystallization
        """
        if not self.facts:
            return {
                'cartridge_id': self.cartridge_id,
                'phase_1_cooccurrence': {'fact_graph_density': 0.0, 'max_co_occurrence_degree': 0},
                'phase_2_query_anchors': {'facts_with_anchors': 0, 'total_anchor_terms': 0},
                'phase_3_ctr': {'avg_confidence': 0.0, 'confidence_distribution': {}},
                'phase_4_seasonality': {'seasonal_facts': 0, 'peak_months': []},
            }
        
        # Phase 1: Co-occurrence graph density
        # Extract from annotations if they contain co-occurrence data
        cooccurrence_edges = 0
        max_degree = 0
        fact_degrees = defaultdict(int)
        
        for fact_id, annotation in self.annotations.items():
            if 'co_occurrence' in annotation:
                cooccur_list = annotation.get('co_occurrence', [])
                cooccurrence_edges += len(cooccur_list)
                fact_degrees[fact_id] += len(cooccur_list)
        
        if fact_degrees:
            max_degree = max(fact_degrees.values())
        
        fact_count = len(self.facts)
        max_possible_edges = fact_count * (fact_count - 1) / 2 if fact_count > 1 else 0
        cooccurrence_density = cooccurrence_edges / max_possible_edges if max_possible_edges > 0 else 0.0
        
        phase_1_stats = {
            'fact_graph_density': round(cooccurrence_density, 4),
            'total_edges': cooccurrence_edges,
            'max_co_occurrence_degree': max_degree,
        }
        
        # Phase 2: Query Anchors
        facts_with_anchors = 0
        total_anchor_terms = 0
        
        for fact_id, annotation in self.annotations.items():
            if 'query_anchors' in annotation:
                anchors = annotation.get('query_anchors', [])
                if anchors:
                    facts_with_anchors += 1
                    total_anchor_terms += len(anchors)
        
        phase_2_stats = {
            'facts_with_anchors': facts_with_anchors,
            'total_anchor_terms': total_anchor_terms,
            'anchor_coverage': round(facts_with_anchors / fact_count, 4) if fact_count > 0 else 0.0,
        }
        
        # Phase 3: CTR (via confidence)
        ...
        for conf in confidences:
            if conf < 0.25:
                ...
                confidence_buckets['0.75-0.95'] += 1
            else:
        ...
        phase_3_stats = {
            ...
            'high_confidence_facts': sum(1 for c in confidences if c >= 0.95),
        }
        ...
        for fact_id, annotation in self.annotations.items():
            if 'seasonality' in annotation:
                seasonality = annotation.get('seasonality', {})
                if seasonality:
                    seasonal_facts += 1
                    peak_month = max(seasonality.items(), key=lambda x: x[1], default=None)
                    if peak_month:
                        peak_months[peak_month[0]] += 1
        
        phase_4_stats = {
            'seasonal_facts': seasonal_facts,
            'seasonal_coverage': round(seasonal_facts / fact_count, 4) if fact_count > 0 else 0.0,
            'peak_months': dict(sorted(peak_months.items(), key=lambda x: x[1], reverse=True)[:5]),
        }
        ...
        return {
            ...
            'phase_1_cooccurrence': phase_1_stats,
            'phase_2_query_anchors': phase_2_stats,
            'phase_3_ctr': phase_3_stats,
            'phase_4_seasonality': phase_4_stats,
        }
