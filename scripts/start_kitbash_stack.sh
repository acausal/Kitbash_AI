...
#
# FIX (2026-07-13): port checks now use `netstat` LISTENING sockets instead of
# `curl` HTTP probes. The old curl probe raced in the launcher's subshell and
# false-negatived on a healthy BitNet, causing a duplicate llama-server to be
# spawned. netstat is curl-independent and matches the listener check used by
# kill_port/verify_venv_webui. The webui is launched detached (nohup & disown,
# no setsid on this host) via run_webui.sh, which self-restarts it on exit so
# the server no longer dies when the launching shell/turn ends.
...
# port_up: 0 if a LISTENING socket exists on $1 (curl-independent; reliable on
# this Windows/git-bash host where curl health probes race).
port_up() {
    local port="$1"
    if netstat -ano 2>/dev/null | grep -qE ":$port +.*LISTENING"; then
        return 0
    fi
    return 1
}
...
# verify a listening :8777 is owned by the venv python (check CommandLine, NOT
...
verify_venv_webui() {
    ...
    if echo "$cmd" | grep -qiE 'Kitbash_AI[\\/]\.venv'; then
        echo "[webui] OK — pid $pid on .venv python"
        return 0
    fi
...
launch_webui() {
    # Detached + self-restarting via run_webui.sh (nohup & disown survives the
    # launching shell's exit; the watchdog loop restarts on crash).
    nohup bash "$REPO/scripts/run_webui.sh" >>/tmp/kitbash_web.log 2>&1 & disown
...
# 1) BitNet inference server (:8080) — skip if already LISTENING (never dup)
if port_up 8080; then
    echo "[bitnet] :8080 already listening — skipping (no duplicate spawn)"
else
    echo "[bitnet] :8080 not listening — launching via start_bitnet.sh (CPU-only, -t 4)"
    nohup bash "$REPO/scripts/start_bitnet.sh" >/tmp/bitnet.log 2>&1 & disown
...
# 3) Web UI (:8777) — under the venv python, detached + watchdog.
# Only (re)launch if not listening, or listening but NOT on the venv python.
if port_up 8777; then
    if verify_venv_webui; then
        echo "[webui] already up on .venv python — leaving it (no clobber)"
    else
        echo "[webui] :8777 up but NOT on .venv — restarting under venv"
        kill_port 8777
        sleep 2
        launch_webui
    fi
else
    echo "[webui] :8777 not listening — launching kitbash_web.py under $VENV_PY"
    launch_webui
fi
